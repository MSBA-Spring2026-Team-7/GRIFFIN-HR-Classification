...
- [x] **A5 -- H2O AutoML training** -- 53 models trained (GBM, DRF, GLM, Deep Learning) on 100 rows x 15 features. 5-fold stratified CV. Best model: GBM (logloss ~0.95, ~48% accuracy, top-3 accuracy ~100%). Variable importance, SHAP (global + local), PDP generated. OpenJDK 25 installed into conda env. Notebook: griffin_h2o_automl.ipynb. Owner: Steven (Apr 6).
